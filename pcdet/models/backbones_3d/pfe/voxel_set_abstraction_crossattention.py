...
def sample_points_with_roi(rois, points, sample_radius_with_roi, num_max_points_of_part=200000):
    ...
    if points.shape[0] < num_max_points_of_part:
        # 计算每个点到每个proposal的欧式距离： (num_of_raw_points, num_of_proposal) .norm是求范数，默认求2范数
        distance = (points[:, None, :] - rois[None, :, 0:3]).norm(dim=-1)
        # 计算每个raw_point距离自己最近proposal的点的距离和索引
    ...
    # 如果所有的特征点都不被采样，则选择第一个点，根据 point_mask 的值选择相应的特征点，并将它们保存在 sampled_points 中。
    # points[point_mask, :] 表示选择 points 中所有 point_mask 中为 True 的元素，并保留所有特征维度
    # 即形状为 (num_sampled_points, num_features) 的张量，其中 num_sampled_points 是被选择的特征点数量。
...
class VoxelSetAbstraction_crossattention(nn.Module):
    def __init__(self, model_cfg, voxel_size, point_cloud_range, num_bev_features=None,
        ...
        self.point_features_linear = nn.Linear(640,128)
        self.point_features_batchnorm = nn.BatchNorm1d(128)
        self.rv_attention = nn.MultiheadAttention(64,8,dropout=0.5)
        self.rb_attention = nn.MultiheadAttention(64,8,dropout=0.5)
        self.bev_linear = nn.Linear(256,64)
        self.voxel_linear = nn.Linear(352,64)
        self.raw_linear = nn.Linear(32,64)
    ...
    def interpolate_from_bev_features(self, keypoints, bev_features, batch_size, bev_stride):
        ...
        # 相当于是坐标系转换了，得到该关键点对应的voxel的x坐标 shape : (2048*batch,)
    ...
    def get_sampled_points(self, batch_dict):
        """
        shape : (batch*2048, 4)  , 4-->batch_idx, x, y, z
        Args:
            batch_dict:
    ...
    def forward(self, batch_dict):
        ...
        if 'bev' in self.model_cfg.FEATURES_SOURCE:
            ...
            point_bev_features = self.interpolate_from_bev_features(
                ...
                # 高度堆叠后的bev特征图的下采样倍数，默认8倍
                bev_stride=batch_dict['spatial_features_stride']
            )
            point_features_list.append(point_bev_features)
            bev_features_attention = point_bev_features
            # print(bev_features_attention)
        ...
        if 'raw_points' in self.model_cfg.FEATURES_SOURCE:
            ...
            pooled_features = self.aggregate_keypoint_features_from_one_source(
                ...
                new_xyz=new_xyz,  # 关键点的shape(2048,3)
                new_xyz_batch_cnt=new_xyz_batch_cnt,  # 一批数据中关键点的个数
            ...
            raw_features_attention = pooled_features
            # print(raw_features_attention)
        ...
        for k, src_name in enumerate(self.SA_layer_names):
            ...
            pooled_features = self.aggregate_keypoint_features_from_one_source(
                ...
                aggregate_func=self.SA_layers[k], # 第K层上的SA网络
                xyz=xyz.contiguous(),  # 所有voxel在点云坐标系下中心点的坐标
                xyz_features=cur_features, # 所有voxel在第K层的特征
                xyz_bs_idxs=cur_coords[:, 0],  # 所有点在batch中的mask
                new_xyz=new_xyz,  # 关键点坐标 (2048, 3)
                new_xyz_batch_cnt=new_xyz_batch_cnt, # 关键点个数
            ...
            point_features_list.append(pooled_features)
            voxel_features_list.append(pooled_features)
        """
                [
        ...
        '''=========把voxel的特征和在一起,32+64+128+128=352'''
        voxel_features_attention = torch.cat(voxel_features_list, dim=-1)
        ...
        # print(bev_features_attention.shape)
        # print(raw_features_attention.shape)
        # print(voxel_features_attention.shape)
        res = self.point_features_batchnorm(self.point_features_linear(point_features))
        bev_features_attention = bev_features_attention.view(-1, bev_features_attention.shape[-1]) # 256
        raw_features_attention = raw_features_attention.view(-1, raw_features_attention.shape[-1]) # 32
        voxel_features_attention = voxel_features_attention.view(-1, voxel_features_attention.shape[-1]) # 352
        # print(bev_features_attention.shape)
        # print(raw_features_attention.shape)
        # print(voxel_features_attention.shape)

        bev_features_attention = self.bev_linear(bev_features_attention).unsqueeze(1)
        raw_features_attention = self.raw_linear(raw_features_attention).unsqueeze(1)
        voxel_features_attention = self.voxel_linear(voxel_features_attention).unsqueeze(1)
        # print(bev_features_attention.shape)
        # print(raw_features_attention.shape)
        # print(voxel_features_attention.shape)

        rb_features,_ = self.rb_attention(bev_features_attention,raw_features_attention,raw_features_attention)
        rv_features,_ = self.rv_attention(voxel_features_attention,raw_features_attention,raw_features_attention)
        rb_features = rb_features.view(-1, rb_features.shape[-1])
        rv_features = rv_features.view(-1, rv_features.shape[-1])
        # print(rb_features.shape)
        # print(rv_features.shape)

        end_features_list = []
        end_features_list.append(rb_features)
        end_features_list.append(rv_features)
        end_features = torch.cat(end_features_list, dim=-1)

        point_features = res + end_features
        # print(point_features.shape)
        ...
        =========
        attention
        point_features_before_fusion是2048*640
        只要最后变成2048*128就可以
        =========
        '''
...
class VoxelSetAbstraction_crossattention_v1(nn.Module):
    def __init__(self, model_cfg, voxel_size, point_cloud_range, num_bev_features=None,
                 num_rawpoint_features=None, **kwargs):
        super().__init__()
        self.model_cfg = model_cfg
        ...
        # self.trans_out = 128
        # self.multihead_attention = MultiHeadAttention(8, self.trans_in, self.trans_out)
        self.voxel_features_linear = nn.Linear(352,128)
        self.voxel_features_batchnorm = nn.BatchNorm1d(128)
        self.bv_attention = nn.MultiheadAttention(256,2,dropout=0.5,kdim=352,vdim=352)
        self.rv_attention = nn.MultiheadAttention(32,2,dropout=0.5,kdim=352,vdim=352)
        self.down_voxel_to_bev = nn.Linear(352,256)
        self.down_voxel_to_raw = nn.Linear(352,32)
        # self.bev_linear = nn.Linear(256,64)
        self.voxel_linear = nn.Linear(352,128)
        # self.raw_linear = nn.Linear(32,64)
    ...
    def forward(self, batch_dict):
        ...
        # print(raw_features_attention.shape)
        # print(voxel_features_attention.shape)
        res = self.voxel_features_batchnorm(self.voxel_features_linear(voxel_features_attention))
        bev_features_attention = bev_features_attention.view(-1, bev_features_attention.shape[-1]) # 256
        raw_features_attention = raw_features_attention.view(-1, raw_features_attention.shape[-1]) # 32
        voxel_features_attention = voxel_features_attention.view(-1, voxel_features_attention.shape[-1]) # 352
        # print(bev_features_attention.shape)
        ...
        bev_features_attention = bev_features_attention.unsqueeze(1)
        raw_features_attention = raw_features_attention.unsqueeze(1)
        voxel_features_attention = voxel_features_attention.unsqueeze(1)
        # print(bev_features_attention.shape)
        # print(raw_features_attention.shape)
        # print(voxel_features_attention.shape)

        down_voxel_to_bev = self.down_voxel_to_bev(voxel_features_attention)
        down_voxel_to_raw = self.down_voxel_to_raw(voxel_features_attention)
        print(down_voxel_to_bev.shape)
        print(down_voxel_to_raw.shape)
        print(bev_features_attention.shape)
        bv_features,_ = self.bv_attention(bev_features_attention,down_voxel_to_bev,voxel_features_attention)
        rv_features,_ = self.rv_attention(raw_features_attention,down_voxel_to_raw,voxel_features_attention)
        # print(bv_features.shape)
        # print(rv_features.shape)
        bv_features = bv_features.view(-1, bv_features.shape[-1])
        rv_features = rv_features.view(-1, rv_features.shape[-1])
        # print(bv_features.shape)
        # print(rv_features.shape)
        ...
        # end_features_list.append(rb_features)
        # end_features_list.append(rv_features)
        end_features = bv_features+rv_features  # 352
        end_features = self.voxel_linear(end_features)
...
class VoxelSetAbstraction_crossattention_v2(nn.Module):
    def __init__(self, model_cfg, voxel_size, point_cloud_range, num_bev_features=None,
                 num_rawpoint_features=None, **kwargs):
        super().__init__()
        self.model_cfg = model_cfg
        ...
        self.voxel_features_linear = nn.Linear(352,128)
        self.voxel_features_batchnorm = nn.BatchNorm1d(128)
        self.bv_attention = DotProductAttention()
        self.rv_attention = DotProductAttention()
        self.down_voxel_to_bev = nn.Linear(352,256)
        self.down_voxel_to_raw = nn.Linear(352,32)
        # self.bev_linear = nn.Linear(256,64)
        self.voxel_linear = nn.Linear(352,128)
    ...
    def forward(self, batch_dict):
        ...
        down_voxel_to_bev = self.down_voxel_to_bev(voxel_features_attention)
        down_voxel_to_raw = self.down_voxel_to_raw(voxel_features_attention)
        bv_features = self.bv_attention(bev_features_attention,down_voxel_to_bev,voxel_features_attention)
        rv_features = self.rv_attention(raw_features_attention,down_voxel_to_raw,voxel_features_attention)
        print(bv_features.shape)
        print(rv_features.shape)
        # bv_features = bv_features.view(-1, bv_features.shape[-1])
        # rv_features = rv_features.view(-1, rv_features.shape[-1])
...
class VoxelSetAbstraction_crossattention_v3(nn.Module):
    def __init__(self, model_cfg, voxel_size, point_cloud_range, num_bev_features=None,
                 num_rawpoint_features=None, **kwargs):
        super().__init__()
        self.model_cfg = model_cfg
        ...
        # self.multihead_attention = MultiHeadAttention(8, self.trans_in, self.trans_out)
        self.voxel_features_batchnorm = nn.BatchNorm1d(128)
        self.bv_attention = nn.MultiheadAttention(352,8,dropout=0.5)
        self.rv_attention = nn.MultiheadAttention(352,8,dropout=0.5)
        self.up_bev_to_voxel = nn.Linear(256,352)
        self.up_raw_to_voxel = nn.Linear(32,352)

        self.voxel_linear = nn.Linear(352,128)
        self.voxel_linear2 = nn.Linear(352,128)
    ...
    def forward(self, batch_dict):
        ...
        '''=========把voxel的特征和在一起,32+64+128+128=352'''
        voxel_features_attention = torch.cat(voxel_features_list, dim=-1)
        # print('voxel_features',voxel_features_attention.shape)

        batch_dict['point_features_before_fusion'] = point_features.view(-1, point_features.shape[-1]) #352

        res = self.voxel_features_batchnorm(self.voxel_linear(voxel_features_attention))
        # print(res.shape)
        bev_features_attention = bev_features_attention.view(-1, bev_features_attention.shape[-1]) # 256
        raw_features_attention = raw_features_attention.view(-1, raw_features_attention.shape[-1]) # 32
        voxel_features_attention = voxel_features_attention.view(-1, voxel_features_attention.shape[-1]) # 352

        # print('bev_features_attention',bev_features_attention.shape)
        # print('raw_features_attention',raw_features_attention.shape)
        # print('voxel_features_attention',voxel_features_attention.shape)
        ...
        up_bev_to_voxel = self.up_bev_to_voxel(bev_features_attention)
        up_raw_to_voxel = self.up_raw_to_voxel(raw_features_attention)
        # print(up_bev_to_voxel.shape)
        # print(up_raw_to_voxel.shape)

        bv_features,_ = self.bv_attention(up_bev_to_voxel,voxel_features_attention,voxel_features_attention)
        rv_features,_ = self.rv_attention(up_raw_to_voxel,voxel_features_attention,voxel_features_attention)
        ...
        end_features = bv_features + rv_features  # 352
        # print('end_features',end_features.shape)
        end_features = self.voxel_linear2(end_features)
        # print(end_features.shape)
...
#     def __init__(self, n_head, d_model_out,**kwargs):
...
#         q_c = q.size(1)
#         # res = self.W_q(q)
#         q = q.view(b_size, num_head, q_c//num_head)
#         # print(q.shape)
...
# class MultiHeadAttention_res(nn.Module):
#     ''' Multi-Head Attention module '''
#     def __init__(self, n_head, d_model_out,**kwargs):
#         super(MultiHeadAttention_res, self).__init__(**kwargs)
#         self.num_head = n_head
#         # self.d_model_in = d_model_in
#         self.d_model_out = d_model_out
#         self.attention = DotProductAttention()
...
#         b_size = q.size(0)
#         # res = self.W_q(q)
#         q = q.view(b_size, num_head, d_model_out//num_head)
#         # print(q.shape)
#         k = self.W_k(k).view(b_size, num_head, d_model_out//num_head)
#         v = self.W_v(v).view(b_size, num_head, d_model_out//num_head)
#         out = self.attention(q,k,v)
