...
class VoxelSetAbstraction(nn.Module):
    ...
    def forward(self, batch_dict):
        ...
        point_features = self.attention(point_features, point_features, point_features)
...
class VoxelSetAbstraction_m3detr(nn.Module):
    def __init__(self, model_cfg, voxel_size, point_cloud_range, num_bev_features=None,
                 num_rawpoint_features=None, **kwargs):
        super().__init__()
        self.model_cfg = model_cfg
        ...
        # add
        self.Linears_in = nn.MoudleList()
        self.Linears_out = nn.MoudleList()
        # x_conv4-> MLPS ->的最后两个维度 64+64=128
        ...
        if 'bev' in self.model_cfg.FEATURES_SOURCE:
            ...
            if c_bev == self.fusion_channel:
                self.Linears_in.append(nn.Identity()) # 占位，没用的
        ...
        if 'raw_points' in self.model_cfg.FEATURES_SOURCE:
            ...
            for k in range(len(mlps)):
                mlps[k] = [num_rawpoint_features -3 ] + mlps[k]  # 为啥？            
            ...
            self.SA_rawpoints = pointnet2_stack_modules.StackSAModuleMSG(
                raddi = SA_cfg['raw_points'].POOL_RADIUS,
                nsamples=SA_cfg['raw_points'].NSAMPLE,
                mlps = mlps,
                use_xyz=True,
                pool_method='max_pool'
            )
            cur = sum([x[-1] for x in mlps])  # 这是啥？ 把mlp最后的输出统计
        ...
        for src_name in self.model_cfg.FEATURES_SOURCE:
            ...
            for k in range(len(mlps)):
                mlps[k] = [mlps[k][0]] + mlps[k]  # 取每个里面的第一个数值加上  ？？
