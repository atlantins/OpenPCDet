...
class AxisAlignedTargetAssigner(object):
    ...
    def assign_targets(self, all_anchors, gt_boxes_with_classes):
        ...
        Args:
            all_anchors: [(N, 7), ...]
            gt_boxes_with_classes: (B, M, 8)  # 最后维度数据为 (x, y, z, l, w, h, θ，class) PVRCNN是(2,26,8)
        Returns:
            all_targets_dict = {
                # 每个anchor的类别
        ...
        # 得到批大小
        batch_size = gt_boxes_with_classes.shape[0]  # 2
        # 得到所有GT的类别
        gt_classes = gt_boxes_with_classes[:, :, -1]  # （2，num_of_gt） num_of_gt = 26,这个是随机的
        # 得到所有GT的7个box参数
        gt_boxes = gt_boxes_with_classes[:, :, :-1]  # （2，num_of_gt，7）
        ...
        for k in range(batch_size):
            ...
            while cnt > 0 and cur_gt[cnt].sum() == 0:  # 找到最后一个不是全为0的
                cnt -= 1
            # 2.1提取当前帧非零的box和类别
            cur_gt = cur_gt[:cnt + 1]
            ...
            for anchor_class_name, anchors in zip(self.anchor_class_names, all_anchors):
                ...
                # anchors : list: 3  (1, 200, 176, 1, 2, 7)  7 --> (x, y, z, l, w, h, θ)
                if cur_gt_classes.shape[0] > 1:
                    # self.class_names : ["car", "person", "cyclist"]
                    # 这里减1是因为列表索引从0开始，目的是得到属于列表中gt中哪些类别是与当前处理的了的类别相同，得到类别mask
                ...
                if self.use_multihead:
                    ...
                    # 2.2.1 计算所需的变量 得到特征图的大小   anchors shape ([1, 200, 176, 1, 2, 7])  feature_map_size ([1, 200, 176])
                    feature_map_size = anchors.shape[:3]
                    # 将所有的anchors展平  (70400, 7)
    ...
    def assign_targets_single(self, anchors, gt_boxes, gt_classes, matched_threshold=0.6, unmatched_threshold=0.45):
        ...
        """
        num_anchors = anchors.shape[0] # 176*200*2=70400
