...
class AnchorGenerator(object):
    def __init__(self, anchor_range, anchor_generator_config):
        super().__init__()
        self.anchor_generator_cfg = anchor_generator_config # car,pedestrian,cyclist 三个类别
        self.anchor_range = anchor_range # [  0.  -40.   -3.   70.4  40.    1. ]
        self.anchor_sizes = [config['anchor_sizes'] for config in anchor_generator_config] # list:3 --> 车、人、自行车[[[3.9, 1.6, 1.56]],[[0.8, 0.6, 1.73]],[[1.76, 0.6, 1.73]]]
        self.anchor_rotations = [config['anchor_rotations'] for config in anchor_generator_config]  # 得到anchor的旋转角度，这是是弧度，也就是0度和90度 list:3 --> [[0, 1.57],[0, 1.57],[0, 1.57]]
        self.anchor_heights = [config['anchor_bottom_heights'] for config in anchor_generator_config]   # 得到每个anchor初始化在点云中z轴的位置，其中在kitti中点云的z轴范围是-3米到1米  list:3 -->  [[-1.78],[-0.6],[-0.6]]
        self.align_center = [config.get('align_center', False) for config in anchor_generator_config]   # 每个先验框产生的时候是否需要在每个格子的中间，例如坐标点为[1,1]，如果需要对齐中心点的话，需要加上0.5变成[1.5, 1.5] 默认为False
    ...
    def generate_anchors(self, grid_sizes):
        ...
        for grid_size, anchor_size, anchor_rotation, anchor_height, align_center in zip(
            ...
            # 2 = 2x1x1 --> 每个位置产生2个anchor，这里的2代表两个方向   grid_size [176,200],grid是特征图尺度大小
            num_anchors_per_location.append(len(anchor_rotation) * len(anchor_size) * len(anchor_height)) # 0,90  * 固定尺寸 len[[里面是一个三维的长宽高]*
            # 2.1计算每个网格的在点云空间中的实际大小
            # 用于将每个anchor映射回实际点云中的大小
            if align_center: # 　不需要对齐中心点来生成先验框 align校准,align_center的意思是已经校准了中心的意思
                x_stride = (self.anchor_range[3] - self.anchor_range[0]) / grid_size[0]  # 用x的范围除以grid_size就是得到步长了
                y_stride = (self.anchor_range[4] - self.anchor_range[1]) / grid_size[1]
                x_offset, y_offset = x_stride / 2, y_stride / 2 # 中心对齐，平移半个网格，就是半个网格的距离，得到整个正方形网格的中心
            else:  # 这里-1，就是计算到每个的中心了，就是不用计算半个网格的偏移了
                # (69.12 - 0) / (176 - 1) = 0.3214883848678234  单位：米
                x_stride = (self.anchor_range[3] - self.anchor_range[0]) / (grid_size[0] - 1)
                # (39.68 - (-39.68.)) / (200 - 1) = 0.3212955490297634  单位：米
            ...
            anchor_rotation = x_shifts.new_tensor(anchor_rotation) #  [0, 1.57] 弧度制
            ...
            # x原来为（176，）-->（176，1, 1）--> (176,200,1)
            # y原来为（200，）--> (1，200，1）--> (176,200,1)
            # z原来为 (1, )  --> (1,1,1)    --> (176,200,1)

            # 2.4.anchor各个维度堆叠组合，生成最终anchor(1,432,496,1,2,7）
            # 2.4.1.堆叠anchor的位置
            # [x, y, z, 3]-->[176, 200, 1, 3] 代表了每个anchor的位置信息
            ...
            # (176, 200, 1, 3) --> (176, 200, 1 , 1, 3)
            # 维度分别代表了： z，y，x， 该类别anchor的尺度数量，该个anchor的位置信息
            anchors = anchors[:, :, :, None, :].repeat(1, 1, 1, anchor_size.shape[0], 1)
            # (1, 1, 1, 1, 3) --> (176, 200, 1, 1, 3)
            anchor_size = anchor_size.view(1, 1, 1, -1, 3).repeat([*anchors.shape[0:3], 1, 1])
            # anchors生成的最终结果需要有位置信息和大小信息 --> (216, 248, 1, 1, 6)  anchor的所有步和anchor的位置信息
            # 最后一个纬度中表示（z, y, x, l, w, h）
            anchors = torch.cat((anchors, anchor_size), dim=-1)

            # 2.4.3.将anchor的位置和大小和旋转角进行组合
            # 在倒数第二个维度上增加一个维度，然后复制该维度一次
            # (176, 200, 1, 1, 2, 6)        长， 宽， 深， anchor尺度数量， 该尺度旋转角个数，anchor的6个参数
            anchors = anchors[:, :, :, :, None, :].repeat(1, 1, 1, 1, num_anchor_rotation, 1)
            # (176, 200, 1, 1, 2, 1)        两个不同方向先验框的旋转角度
            anchor_rotation = anchor_rotation.view(1, 1, 1, 1, -1, 1).repeat([*anchors.shape[0:3], num_anchor_size, 1, 1])
            # [z, y, x, num_size, num_rot, 7] --> (176, 200, 1, 1, 2, 7)
            # 最后一个纬度表示为anchors的位置+大小+旋转角度（z, y, x, l, w, h, theta）
            anchors = torch.cat((anchors, anchor_rotation), dim=-1)  # [x, y, z, num_size, num_rot, 7]，因为之前是None所以这里可以cat
