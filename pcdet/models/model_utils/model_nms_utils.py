...
def class_agnostic_nms(box_scores, box_preds, nms_config, score_thresh=None): # 1.首先根据置信度阈值过滤掉部过滤掉大部分置信度低的box，加速后面的nms操作
    src_box_scores = box_scores
    if score_thresh is not None:
        scores_mask = (box_scores >= score_thresh)  # 得到类别预测概率大于score_thresh的mask，相当于挑选置信度高的
        box_scores = box_scores[scores_mask]
        box_preds = box_preds[scores_mask]

    selected = []  # 初始化空列表，用来存放经过nms后保留下来的anchor
    if box_scores.shape[0] > 0:
        box_scores_nms, indices = torch.topk(box_scores, k=min(nms_config.NMS_PRE_MAXSIZE, box_scores.shape[0])) # 保留k个进行nms操作
    ...
    if score_thresh is not None: # 如果存在置信度阈值，scores_mask是box_scores在src_box_scores中的索引，即原始索引
